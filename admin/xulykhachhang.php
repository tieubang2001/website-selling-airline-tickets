...
<?php
	if(isset($_POST['themkhachhang'])){
		...
		$sql_insert_khachhang = mysqli_query($con,"INSERT INTO tbl_khachhang(name,cccd,phone,address,note,email,password) values ('$name','$cccd','$phone','$address','$note','$email','$password')");}
	elseif(isset($_POST['capnhatkhachhang'])) {
		...
		$email = $_POST['email'];
		$password = md5($_POST['password']);
		$sql_update_khachhang = mysqli_query($con,"UPDATE tbl_khachhang SET name='$name',cccd='$cccd',phone='$phone',address='$address',note='$note',email='$email' ,password='$password' WHERE khachhang_id='$id_update'");
		...
		<?php
			if(isset($_GET['quanly'])=='capnhat'){
				...
				<form action="" method="POST">
					...
					<label>CCCD</label>
					<input type="text" class="form-control" name="cccd" value="<?php echo $row_capnhat['cccd'] ?>"><br>
			...
			<?php
				...
				<form action="" method="POST">	
					<label>Tên Khách Hàng</label>
					<input type="text" class="form-control" name="name" placeholder="Họ tên" ><br>
					<label>CCCD</label>
					<input type="text" class="form-control" name="cccd" placeholder="CCCD" ><br>
					<label>Số điện thoại</label>
			...
			<div class="col-md-8">
				...
				<table class="table table-bordered ">
					...
					<tr>
						...
						<td><?php echo $row_khachhang['cccd']; ?></td>
